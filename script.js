...
function startGame(m) {
  ...
  document.getElementById('methodTitle').innerText = 
    method==='memory'?'Memory-Spiel':method==='flashcards'?'Karteikarten':'Wörter lernen';
  setupGame();
}
...
function setupMemory() {
  ...
  board.innerHTML = '';
  const allWords = [...words[language]].slice(0,8); // 8 Paare
  let cards = [];
...
function checkMatch() {
  if(firstCard.dataset.back === secondCard.dataset.back) {
    points +=10;
    firstCard.classList.add('correct');
    secondCard.classList.add('correct');
    resetCards();
    matchedPairs++;
    if(matchedPairs === totalPairs) alert('Glückwunsch! Du hast alle Paare gefunden!');
  } else {
    ...
    setTimeout(()=>{
      ...
      firstCard.innerText = firstCard.dataset.front;
      secondCard.classList.remove('flipped','wrong');
      secondCard.innerText = secondCard.dataset.front;
      resetCards();
    },1000);
  }
  updatePointsAndRank();
